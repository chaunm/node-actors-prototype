...
```js
{
  ...
  params: {
    error: error.{command.<command>.timeout, command.<command>.error, simcard}
    error_message: <message that describes error detail in case of command error, normally generated by gsm device>
  }
}
```
